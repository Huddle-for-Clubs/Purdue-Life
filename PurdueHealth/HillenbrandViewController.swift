//
//  HillenbrandViewController.swift
//  PurdueHealth
//
...
class HillenbrandViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    
    @IBOutlet weak var mealLabel: UILabel!
    @IBOutlet weak var windsorLunchTableView: UITableView!
    var items: [[Item?]] = []
    var sectionTitles: [String] = []
    let baseURL = "http://api.hfs.purdue.edu/menus/v1/locations/Hillenbrand/01-10-17"
    
    override func viewDidLoad() {
        super.viewDidLoad()
        let defaults = UserDefaults.standard
        defaults.set(1,forKey: "hillenbrandMeal")
    ...
    func getData(){
        ...
        Alamofire.request(baseURL,method: .get,encoding: URLEncoding.default,headers: ["Accept" : "application/json"]).validate().responseJSON{response in
            if response.result.isSuccess{
                ...
                let mealValue = defaults.integer(forKey: "hillenbrandMeal")
    ...
    @IBAction func onLeft(_ sender: Any) {
        ...
        switch mealValue {
        case 1:
            defaults.set(0,forKey: "hillenbrandMeal")
            defaults.synchronize()
            getData()
            self.mealLabel.text = "Breakfast"
        case 0:
            defaults.set(2,forKey: "hillenbrandMeal")
            defaults.synchronize()
            getData()
